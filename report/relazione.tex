...
La Random Forest raggiunge l'accuratezza e la precisione pi\`u alte, ma \`e la Regressione Logistica a ottenere la recall pi\`u elevata (0.797): individua quindi la quota maggiore di clienti che abbandonano realmente, a fronte di un numero maggiore di falsi positivi. Il k-NN ottiene le prestazioni pi\`u basse tra i tre modelli; la recall inferiore pu\`o essere in parte legata all'assenza di un meccanismo nativo di bilanciamento delle classi, oltre che alla sensibilit\`a del k-NN alla struttura locale dei dati e alla scelta di $k$.
...
La Figura~\ref{fig:importance} mostra le dieci variabili con maggiore \emph{feature importance} secondo la Random Forest. Le tre variabili pi\`u rilevanti sono \texttt{TotalCharges} (0.176), \texttt{tenure} (0.169) e \texttt{MonthlyCharges} (0.150), coerentemente con la collinearit\`a tra queste variabili gi\`a osservata in Sezione~\ref{sec:metodologia}. Va per\`o precisato che, in presenza di variabili fortemente correlate come queste, l'importanza calcolata dalla Random Forest pu\`o ripartirsi tra le variabili che condividono informazione simile: il fatto che \texttt{TotalCharges} compaia al primo posto non implica quindi, in senso assoluto o causale, che sia la determinante pi\`u forte del churn. Il tipo di contratto, pur comparendo pi\`u in basso in questa classifica, resta rilevante: avere un contratto biennale (\texttt{Contract\_Two~year}) \`e la quarta feature per importanza ed \`e anche uno dei coefficienti pi\`u negativi -- quindi pi\`u protettivi -- nella Regressione Logistica. Il coefficiente positivo pi\`u forte nella Regressione Logistica \`e invece \texttt{InternetService\_Fiber~optic} (+0.78), coerente con quanto osservato nell'analisi esplorativa (Sezione~\ref{sec:eda}): i clienti con fibra ottica abbandonano molto pi\`u spesso. Il fatto che due modelli molto diversi tra loro (uno lineare, uno basato su alberi) concordino sui fattori principali rafforza la robustezza descrittiva del risultato, pur trattandosi in entrambi i casi di associazioni statistiche e non di relazioni causali dimostrate.
